//! This is simply a copy of: https://doc.rust-lang.org/nightly/nightly-rustc/rustc_data_structures/sso/enum.SsoHashMap.html
//!
//! For use outside of rustc
...
impl<K: Eq + Hash, V> SsoHashMap<K, V> {
    ...
    pub fn remove(&mut self, key: &K) -> Option<V> {
        match self {
            SsoHashMap::Array(array) => array
                .iter()
                .position(|(k, _v)| k == key)
                .map(|index| array.swap_remove(index).1),
    ...
    pub fn remove_entry(&mut self, key: &K) -> Option<(K, V)> {
        match self {
            SsoHashMap::Array(array) => array
                .iter()
                .position(|(k, _v)| k == key)
                .map(|index| array.swap_remove(index)),
